...
extern int act8931_charge_charge_level_set(int level);
#endif
...
#define  BAT_DEFINE_VALUE                                         3200
#else
#define  BAT_DEFINE_VALUE	                                     2500
#endif

#define GPIO_CHGLEV_PIN RK30_PIN1_PA1
#ifdef CONFIG_USE_EBC_SDK_DISPBOARD
#define GPIO_ACIN_PIN INVALID_GPIO
#else
#define GPIO_ACIN_PIN RK30_PIN1_PB2
#endif
...
static int rk_battery_get_status(struct rk30_adc_battery_data *bat)
{
	...
	if(bat->bat_voltage < BATT_ZERO_VOL_VALUE){
			printk("low power\n");
			wake_lock_timeout(&batt_wake_lock,15*HZ);
			bat ->bat_capacity=0;
		//	bat->bat_change  = 1;
...
static int rk30_adc_battery_voltage_to_capacity(struct rk30_adc_battery_data *bat, int BatVoltage)
{
	...
	if (1 == bat->charge_level){  //charge
		if(0 == bat->start_voltage_status ){
			...
			DBG("BatVoltage=%d,start_voltage=%d,start_capacity =%d\n",BatVoltage,bat->charge_start_voltage, bat->charge_start_capacity);
...
//	int timer_of_discharge_sample = NUM_CHARGE_MIN_SAMPLE;
	...
	if(1 == bat->charge_level){
		...
		if(1 == bat->usb_charging){
			#if defined(CONFIG_REGULATOR_ACT8931_CHARGE)
			act8931_charge_charge_level_set(2);
			#endif
			...
			#if defined(CONFIG_REGULATOR_ACT8931_CHARGE)
			act8931_charge_charge_level_set(4);
			#endif
			rk_ac_charger(bat);
			}
#else
...
1111			3.8
*/
#define NORMORL_VOL (0xC)
extern int batlow_irq_config(int syslev);

static void batt_low(struct work_struct *work)
{
	struct rk30_adc_battery_data *bat;

	enable_irq(gpio_to_irq(bat ->pdata->batt_low_pin));
...
#if defined(CONFIG_REGULATOR_ACT8931_BATLOW)	
	int syslev = NORMORL_VOL;

	batlow_irq_config(NORMORL_VOL);
	while(gpio_get_value (data ->pdata->batt_low_pin)==data ->pdata->batt_low_level)
	{
		if(syslev-- > 0)
			batlow_irq_config(syslev);
		else
		{
			printk("\ntoo low battery, shutdown now\n");
			//rk30_adc_battery_resume_check();
			wake_lock_timeout(&batt_wake_lock,10*HZ);
			data ->bat_capacity=0;
			power_supply_changed(&data ->bat);
			//rk28_send_wakeup_batlow();
			return -1;
		}
	}
	if(syslev != NORMORL_VOL)
	{
		batlow_irq_config(NORMORL_VOL);
		if(gpio_get_value (data ->pdata->batt_low_pin)==data ->pdata->batt_low_level)
		{
			printk("too low battery, shutdown now\n");
			//rk30_adc_battery_resume_check();
			//wake_lock_timeout(&batt_wake_lock,10*HZ);
...
static int rk30_adc_battery_io_init(struct rk30_adc_battery_platform_data *pdata)
{
	...
	//charge control pin
	if (pdata->charge_set_pin != INVALID_GPIO){
	    	ret = gpio_request(pdata->charge_set_pin, "chg_sel");
	    	if (ret) {
	...
	if (pdata->dc_det_pin != INVALID_GPIO){
	    	...
	    	gpio_pull_updown(pdata->dc_det_pin, GPIO_LOW);//important
	...
	//charge ok detect
	if (pdata->charge_ok_pin != INVALID_GPIO){
 		ret = gpio_request(pdata->charge_ok_pin, "chg_ok");
	    	if (ret) {
	    		printk("failed to request charge_ok gpio\n");
	    		goto error;
	    	}
	
	    	gpio_pull_updown(pdata->charge_ok_pin, PullEnable);//important
	...
	if( pdata->batt_low_pin != INVALID_GPIO){
 		ret = gpio_request(pdata->batt_low_pin, "bat_low");
	    	if (ret) {
	    		printk("failed to request batt_low_pin gpio\n");
	    		goto error;
	    	}
	
	    	gpio_pull_updown(pdata->batt_low_pin, PullEnable); 
...
int rk30_adc_battery_get_bat_vol(void)
{
	rk30_adc_battery_voltage_samples(gBatteryData);//get voltage
	DBG("gBatteryData->bat_voltage:%d\n",gBatteryData->bat_voltage);

	return (gBatteryData->bat_voltage >= BATT_ZERO_VOL_VALUE)?0:1;
}

void rk30_adc_ebc_battery_check(void)
{
	if(rk30_adc_battery_get_bat_vol()){
		int gpio_acin_lev,gpio_chglev_lev;
	
		gpio_acin_lev = gpio_get_value(GPIO_ACIN_PIN);
		gpio_chglev_lev = gpio_get_value(GPIO_CHGLEV_PIN);

		gpio_set_value(GPIO_ACIN_PIN,GPIO_HIGH);
		gpio_set_value(GPIO_CHGLEV_PIN,GPIO_HIGH);
		
		while(rk30_adc_battery_get_bat_vol()){
			if(rk_adc_battery_poweron_status(gBatteryData)==0){
				while(1){
		...
		gpio_direction_output(GPIO_ACIN_PIN, gpio_acin_lev);
		gpio_direction_output(GPIO_CHGLEV_PIN, gpio_chglev_lev);
	}
}
...
//	pSamples = bat->adc_samples;
	...
	if(level)
	{
		...
		if (bat->bat_voltage <= BATT_ZERO_VOL_VALUE)
		{
			printk("too low battery: charging...\n");
			while(1){
				if(level== 0){
					while(1){
						if(gpio_get_value(POWER_ON_PIN) == GPIO_HIGH){
	...
	/*else if(bat->bat_voltage < BATT_ZERO_VOL_VALUE)
	{
		printk("too low battery: powerdown\n");
		while(1){
			if(gpio_get_value(POWER_ON_PIN) == GPIO_HIGH){
				gpio_set_value(POWER_ON_PIN,GPIO_LOW);
			}
...
//	data->wq = create_singlethread_workqueue("adc_battd");
	...
	if( pdata->batt_low_pin != INVALID_GPIO){
		...
		irq = gpio_to_irq(pdata->batt_low_pin);		
	    	ret = request_irq(irq, rk30_adc_battery_low_wakeup, IRQF_TRIGGER_FALLING, "batt_low_irq", NULL);
	    	if (ret) {
			ret = -EINVAL;
