...
static void rk29_sdmmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    //if(host->bus_mode != ios->power_mode)
    {
        switch (ios->power_mode) 
        {
			...
			//power-on; 
			if(host->gpio_power_en != INVALID_GPIO)
			{
				gpio_direction_output(host->gpio_power_en, host->gpio_power_en_level);
			}
			else if(host->sd_regulator != NULL)
			{
				ldo_sd = regulator_get(NULL, host->sd_regulator);	// vcc_sd
			...
			{
				ldo_sd = regulator_get(NULL, host->sd_regulator);
				while(regulator_is_enabled(ldo_sd)>0)
...
static int rk29_sdmmc_probe(struct platform_device *pdev)
{
    ...
    host->sd_regulator = pdata->regulator;

    host->set_iomux = pdata->set_iomux;

    if (host->sd_regulator != NULL) {
	    ldo_sd = regulator_get(NULL, host->sd_regulator);
	    regulator_enable(ldo_sd);
	    regulator_put(ldo_sd);
	    mdelay(5);
