...
int get_lcdc_display_addr(struct rk_lcdc_device_driver *dev_drv)
{
	...
	int addr;
	addr = lcdc_readl(lcdc_dev,WIN0_YRGB_MST0);
	return addr;
}
int rk3188_lcdc_reg_ebc_update(struct rk_lcdc_device_driver *dev_drv)
{
	...
	int ret;
	int dsp_stx,dsp_sty;
	dsp_stx = dev_drv->cur_screen->hsync_len + dev_drv->cur_screen->left_margin;//LcdReadBit(inf, DSP_HACT_ST, m_BIT11LO);
	dsp_sty = dev_drv->cur_screen->vsync_len +dev_drv->cur_screen->upper_margin;
	if(likely(lcdc_dev->clk_on))
	{
		...
		lcdc_msk_reg(lcdc_dev, SYS_CTRL, m_WIN0_EN | m_WIN1_EN | m_WIN0_RB_SWAP | m_WIN1_RB_SWAP, 
			v_WIN0_EN(1) | v_WIN1_EN(0) | v_WIN0_RB_SWAP(0) |
			v_WIN1_RB_SWAP(0));
		lcdc_writel(lcdc_dev,WIN0_SCL_FACTOR_YRGB,v_X_SCL_FACTOR(0x1000) | 
			v_Y_SCL_FACTOR(0x1000));
		lcdc_writel(lcdc_dev,WIN0_SCL_FACTOR_CBR,v_X_SCL_FACTOR(0x1000) | 
			v_Y_SCL_FACTOR(0x1000));
		lcdc_msk_reg(lcdc_dev,SYS_CTRL,m_WIN0_FORMAT,v_WIN0_FORMAT(2));		//(inf->video_mode==0)
		lcdc_writel(lcdc_dev,WIN0_ACT_INFO,v_ACT_WIDTH(win0->xsize) | v_ACT_HEIGHT(win0->ysize));
		lcdc_writel(lcdc_dev,WIN0_DSP_ST,v_DSP_STX(dsp_stx) | v_DSP_STY(dsp_sty));
		lcdc_writel(lcdc_dev,WIN0_DSP_INFO,v_DSP_WIDTH(win0->xsize) | v_DSP_HEIGHT(win0->ysize));
		lcdc_msk_reg(lcdc_dev,WIN_VIR,m_WIN0_VIR,v_WIN0_VIR_VAL(v_RGB565_VIRWIDTH(win0->xsize)));
		lcdc_writel(lcdc_dev,WIN0_YRGB_MST0, win0->y_addr);
		lcdc_writel(lcdc_dev,WIN0_CBR_MST0, win0->uv_addr);
		lcdc_cfg_done(lcdc_dev);
		//rk3188_lcdc_reg_dump(lcdc_dev);
		lcdc_msk_reg(lcdc_dev,MCU_CTRL,m_MCU_FS_HOLD_STA  ,
			  v_MCU_FS_HOLD_STA(1) );
		
	}
	return 0;
}
int register_ebc_lcdc_ops(struct ebc_lcdc_ops *ops)
{
	ops->get_lcdc_display_addr = get_lcdc_display_addr;
	ops->rk3188_lcdc_reg_ebc_update = rk3188_lcdc_reg_ebc_update;
	ops->rk3188_lcdc_clk_enable = rk3188_lcdc_clk_enable;
	ops->rk3188_lcdc_clk_disable = rk3188_lcdc_clk_disable;
}
#endif
...
static int rk3188_load_screen(struct rk_lcdc_device_driver *dev_drv, bool initscreen)
{
	...
	if(likely(lcdc_dev->clk_on))
	{
		...
		{
	    		lcdc_msk_reg(lcdc_dev, MCU_CTRL, m_MCU_TYPE,v_MCU_TYPE(1));
			// set out format and mcu timing
			if(screen->face == OUT_P16BPP4){
				mcu_total = screen->mcu_wrperiod;//9; //7
				mcu_rwstart = 0;//0
				mcu_csstart = 0;
				mcu_rwend = screen->mcu_wrperiod/2;// 4;///5;
				mcu_csend = screen->mcu_wrperiod/2;//5;
			}
			else{
		   		mcu_total  = (screen->mcu_wrperiod*screen->pixclock*1000)/1000000;
		    		if(mcu_total>31)    
					mcu_total = 31;
		   		if(mcu_total<3)    
					mcu_total = 3;
			...
			if(screen->face != OUT_P16BPP4){
				...
				screen->pixclock = screen->pixclock*1000000;
			}
			lcdc_msk_reg(lcdc_dev, MCU_CTRL,m_MCU_CS_ST | m_MCU_CS_END| m_MCU_RW_ST | m_MCU_RW_END |
	             		m_MCU_PIX_TOTAL | m_MCU_HOLD_MODE | m_MCU_FS_HOLD_STA,
	            		v_MCU_CS_ST(mcu_csstart) | v_MCU_CS_END(mcu_csend) | v_MCU_RW_ST(mcu_rwstart) |
	            		v_MCU_RW_END(mcu_rwend) |  v_MCU_PIX_TOTAL(mcu_total) |
	            		v_MCU_HOLD_MODE((SCREEN_MCU==screen->type)?(1):(0)) | v_MCU_FS_HOLD_STA(0));
		...
		case OUT_P16BPP4:
			face = OUT_P565;  //dither down to rgb565
			lcdc_msk_reg(lcdc_dev, DSP_CTRL0, m_DITHER_DOWN_EN | m_DITHER_DOWN_MODE |
				 m_DITHER_DOWN_SEL,v_DITHER_DOWN_EN(0) | v_DITHER_DOWN_MODE(0) |
				 v_DITHER_DOWN_SEL(0));
			break;
		default:
...
static int rk3188_lcdc_early_suspend(struct rk_lcdc_device_driver *dev_drv)
{
	...
	if(dev_drv->screen_ctr_info->prop == EBC_EXTEND)
		return 0;
...
#if (defined(CONFIG_ARCH_RK3026) && !defined(CONFIG_EBC))
...
static irqreturn_t rk3188_lcdc_isr(int irq, void *dev_id)
{
	...
	struct rk_lcdc_device_driver * dev_drv =&lcdc_dev->driver;
	...
	if(int_reg & m_FS_INT_STA)
	{
		...
		dev_drv-> mcuisr_cnt = !dev_drv-> mcuisr_cnt;
		 if(dev_drv-> mcuisr_cnt)   
		 	return IRQ_HANDLED;
		 if(dev_drv->mcuisr_callback)
		 	dev_drv->mcuisr_callback();
		 return IRQ_HANDLED;
		}
