...
static int rk_fb_set_par(struct fb_info *info)
{
	...
	#if defined(CONFIG_RK_HDMI)
		#if 0 // defined(CONFIG_DUAL_LCDC_DUAL_DISP_IN_KERNEL)
			if(hdmi_get_hotplug() == HDMI_HPD_ACTIVED)
			{
				struct rk_fb_inf *inf = dev_get_drvdata(info->device);
...
static int rk_request_fb_buffer(struct fb_info *fbi,int fb_id)
{
	...
	{	
#if defined(CONFIG_FB_ROTATE) || !defined(CONFIG_THREE_FB_BUFFER) || defined(CONFIG_DUAL_LCDC_DUAL_DISP_IN_KERNEL)
...
static int __devinit rk_fb_probe (struct platform_device *pdev)
{
	...
	ldo_lcdc = regulator_get(NULL, "vcc_lcd");
	if(ldo_lcdc){
