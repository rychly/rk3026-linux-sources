...
void set_lcd_info(struct rk29fb_screen *screen, struct rk29lcd_info *lcd_info )
{
	...
	screen->swap_delta = 0;
	screen->swap_dumy = 0;
	#elif defined(CONFIG_HARDWARE_EBC)
	...
	set_epd_info(NULL,NULL,&screen->x_res,&screen->y_res);
...
size_t get_fb_size(void)
{
	...
	int width,height;
	#if defined(CONFIG_EBC)
		set_epd_info(NULL,NULL,&width,&height);
	...
	#if defined(CONFIG_THREE_FB_BUFFER)
		size = ((width)*(height)<<2)* 3; //three buffer
	#else
		size = ((width)*(height)<<2)<<1; //two buffer
	#endif
	return ALIGN(size,SZ_1M);
}
#endif
