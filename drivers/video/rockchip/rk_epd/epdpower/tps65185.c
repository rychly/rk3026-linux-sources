...
static int papyrus_probe(struct pmic_sess *pmsess,struct i2c_client *client)
{
	...
	do_gettimeofday(&sess->standby_tv);
	sess->wake_up_pin = pdata->wake_up_pin;
	sess->vcom_ctl_pin = pdata->vcom_ctl_pin;
...
static int tps65185_suspend(struct i2c_client *client)
{
...
static ssize_t vcom_mv_get(struct device_driver *dev, char *buf)
{
...
static ssize_t vcom_mv_set(struct device_driver *dev, const char *_buf, size_t _count)
{
	int value;
...
static DRIVER_ATTR(vcom_mv, 0666, vcom_mv_get, vcom_mv_set);
...
static int __init tps65185_init(void)
{
	...
	ret = driver_create_file(&tps65185_driver.driver, &driver_attr_vcom_mv);
...
static int tps65185_power_on()
{
	if(pmic_sess_data.is_inited)
		pmic_driver_tps65185_i2c.hw_power_req((struct pmic_sess *)&pmic_sess_data,1);
	return 0;
}
static int tps65185_power_down()
{
	if(pmic_sess_data.is_inited)
