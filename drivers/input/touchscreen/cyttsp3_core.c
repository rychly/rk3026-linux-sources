...
extern struct fb_info *g_fb0_inf;  //add cym@rk 20101027 for charger logo*/
...
static void handle_multi_touch(struct cyttsp_track_data *t, struct cyttsp *ts)
{
	...
	for (id = 0; id < CY_NUM_TRK_ID; id++) {
		...
		LOG("%s:1 X:% 3d,Y:% 3d,Z:% 3d  ID:% 2d\n",
				__func__, ts->prv_mt_pos[id][CY_XPOS], ts->prv_mt_pos[id][CY_YPOS], CY_NTCH,
				id);
	}
	...
	for (id = 0; id < CY_NUM_MT_TCH_ID; id++) {
		if (t->cur_mt_tch[id] >= CY_NUM_TRK_ID)
			continue;
		input_mt_slot(ts->input, t->cur_mt_tch[id]);		
		input_report_abs(ts->input, ABS_MT_TRACKING_ID, t->cur_mt_tch[id]);
		input_report_abs(ts->input, ABS_MT_POSITION_X, t->cur_mt_pos[id][CY_XPOS]);
		input_report_abs(ts->input, ABS_MT_POSITION_Y, t->cur_mt_pos[id][CY_YPOS]);
		...
		LOG("%s:2 X:% 3d,Y:% 3d,Z:% 3d,width:% 3d, ID:% 2d\n",
				__func__, t->cur_mt_pos[id][CY_XPOS], t->cur_mt_pos[id][CY_YPOS], t->cur_mt_z[id],
				t->tool_width, t->cur_mt_tch[id]);
	}
	return;
no_track_id:
	...
	for (id = 0; id < CY_NUM_MT_TCH_ID; id++) {
		...
		if (t->snd_trk[id] < CY_NUM_TRK_ID) {
			...
			#if defined(CONFIG_MACH_RK29_T61) || defined(CONFIG_MACH_RK3026_TB)
			input_report_abs(ts->input, ABS_MT_POSITION_X,
					ts->platform_data->maxx - t->cur_mt_pos[t->snd_trk[id]][CY_XPOS]);
			input_report_abs(ts->input, ABS_MT_POSITION_Y,
					ts->platform_data->maxy - t->cur_mt_pos[t->snd_trk[id]][CY_YPOS]);
			#else
			input_report_abs(ts->input, ABS_MT_POSITION_X, t->cur_mt_pos[t->snd_trk[id]][CY_XPOS]);
			input_report_abs(ts->input, ABS_MT_POSITION_Y, t->cur_mt_pos[t->snd_trk[id]][CY_YPOS]);
			#endif
			input_report_abs(ts->input, ABS_MT_WIDTH_MAJOR,	5);
			...
			#if defined(CONFIG_MACH_RK29_T61) || defined(CONFIG_MACH_RK3026_TB)
			input_report_abs(ts->input, ABS_MT_POSITION_X,
				ts->platform_data->maxx - ts->prv_mt_pos[ts->prv_mt_tch[id]][CY_XPOS]);
			input_report_abs(ts->input, ABS_MT_POSITION_Y,
				ts->platform_data->maxy - ts->prv_mt_pos[ts->prv_mt_tch[id]][CY_YPOS]);
			#else
			input_report_abs(ts->input, ABS_MT_POSITION_X,
...
static void cyttsp_xy_worker(struct cyttsp *ts)
{
	...
	case 4:
		...
		if (!memcmp(ts->bus_ops->touchscreen_type, TOUCHSCREEN_XC3, sizeof(TOUCHSCREEN_XC3))) {
			if (tilt)
		...
		} else if (!memcmp(ts->bus_ops->touchscreen_type, TOUCHSCREEN_OED, sizeof(TOUCHSCREEN_OED))) {
			if (tilt)
				FLIP_XY(xy_data.x4, xy_data.y4);
			temp = xy_data.x4*g_fb0_inf->var.xres;
			xy_data.x4 = temp>>11; // X/(2^11)
			temp = xy_data.y4*g_fb0_inf->var.yres;
			xy_data.y4 = temp>>11; // Y/(2^11)
		}
	...
	case 3:
		...
		if (!memcmp(ts->bus_ops->touchscreen_type, TOUCHSCREEN_XC3, sizeof(TOUCHSCREEN_XC3))) {
			...
			if (tilt)
				FLIP_XY(xy_data.x3, xy_data.y3);
			temp = xy_data.x3*g_fb0_inf->var.xres;
			xy_data.x3 = temp>>11; // X/(2^11)
			temp = xy_data.y3*g_fb0_inf->var.yres;
			xy_data.y3 = temp>>11; // Y/(2^11)
		}
	...
	case 2:
		...
		if (!memcmp(ts->bus_ops->touchscreen_type, TOUCHSCREEN_XC3, sizeof(TOUCHSCREEN_XC3))) {
			...
			if (tilt)
				FLIP_XY(xy_data.x2, xy_data.y2);
			temp = xy_data.x2*g_fb0_inf->var.xres;
			xy_data.x2 = temp>>11; // X/(2^11)
			temp = xy_data.y2*g_fb0_inf->var.yres;
			xy_data.y2 = temp>>11; // Y/(2^11)
	...
	case 1:
		...
		if (!memcmp(ts->bus_ops->touchscreen_type, TOUCHSCREEN_XC3, sizeof(TOUCHSCREEN_XC3))) {
			...
			if (tilt)
				FLIP_XY(xy_data.x1, xy_data.y1);
			temp = xy_data.x1*g_fb0_inf->var.xres;
			xy_data.x1 = temp>>11; // X/(2^11)
			temp = xy_data.y1*g_fb0_inf->var.yres;
			xy_data.y1 = temp>>11; // Y/(2^11)
		}
...
static irqreturn_t cyttsp_irq(int irq, void *handle)
{
	...
	case CY_ACTIVE_STATE:
		wake_lock_timeout(&ts->tp_lock, 2 * HZ);
		cyttsp_xy_worker(ts);
		break;
	case CY_READY_STATE:
	default:
...
static void cyttsp_ts_early_suspend(struct early_suspend *h)
{
	...
	DBG(printk(KERN_INFO"%s: Enter\n", __func__);)
	LOCK(ts->mutex);
	pm_sleep_status = get_suspend_state();
	if(pm_sleep_status == PM_SUSPEND_IDLE)
		goto out;
...
void *cyttsp_core_init(struct cyttsp_bus_ops *bus_ops, struct device *pdev)
{
	...
	*/
    ...
    input_device->absbit[0] = BIT(ABS_X) | BIT(ABS_Y) | BIT(ABS_PRESSURE);
	...
	if (!memcmp(ts->bus_ops->touchscreen_type, TOUCHSCREEN_XC3, sizeof(TOUCHSCREEN_XC3))) {
		...
		input_set_abs_params(input_device, ABS_X, 0, ts->platform_data->oed_maxx, 0, 0);
		input_set_abs_params(input_device, ABS_Y, 0, ts->platform_data->oed_maxy, 0, 0);
		input_set_abs_params(input_device, ABS_HAT0X, 0, ts->platform_data->oed_maxx, 0, 0);
		input_set_abs_params(input_device, ABS_HAT0Y, 0, ts->platform_data->oed_maxy, 0, 0);
	}
	...
	if (ts->platform_data->use_mt) {
		if (!memcmp(ts->bus_ops->touchscreen_type, TOUCHSCREEN_XC3, sizeof(TOUCHSCREEN_XC3))) {
			...
			input_set_abs_params(input_device, ABS_MT_POSITION_X, 0, ts->platform_data->oed_maxx, 0, 0);
			input_set_abs_params(input_device, ABS_MT_POSITION_Y, 0, ts->platform_data->oed_maxy, 0, 0);
		}
	...
	device_init_wakeup(ts->pdev, 1);

	wake_lock_init(&ts->tp_lock, WAKE_LOCK_SUSPEND, "tp_lock");
