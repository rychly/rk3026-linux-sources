...
#define CYPRESS_RST_PORT                RK30_PIN3_PC1
#define CYPRESS_INT_PORT                RK30_PIN1_PB0 
#define CYPRESS_INT_IRQ(int_gpio)       gpio_to_irq(int_gpio)
...
#define cyttsp_alert(f, a...)		pr_warning("%s:" f,  __func__ , ## a)

#ifdef CY_USE_DEBUG
	#define cyttsp_debug(f, a...)	pr_info("%s:" f,  __func__ , ## a)
#else
	#define cyttsp_debug(f, a...)	{if (cyttsp_tsdebug) pr_alert("%s:" f,  __func__ , ## a); }
#endif /* CY_USE_DEBUG */

#ifdef CY_ALLOW_EXTRA_DEBUG
    #ifdef CY_USE_EXTRA_DEBUG
    	#define cyttsp_xdebug(f, a...)	pr_warning("%s:" f,  __func__ , ## a)
    #else
...
#define GET_BUFFERSTATUS(reg)   ((reg & 0x20) >> 5)
#define GET_DATACOUNTER(reg)    ((reg & 0xC0) >> 6)

/* constant definitions */
/* maximum number of concurrent ST track IDs */
#define CY_NUM_ST_TCH_ID		8

/* maximum number of concurrent MT track IDs */
#define CY_NUM_MT_TCH_ID	    10
...
#define CY_REG_TT_MODE          0x01
#define CY_REG_TOUCH1_DATA      0x03
...
#define CY_ACT_INTRVL		    0x00

/* touch timeout for the Active power */
#define CY_TCH_TMOUT		    0xFF

/* Low Power state scanning/processing refresh interval */
#define CY_LP_INTRVL		    0x00

#define CY_GEST_SET             0x05
...
struct cyttsp_platform_data {
	...
	u8 lp_intrvl;
	u8 power_state;
    struct cyttsp_board_data board_data;
#ifdef CY_USE_I2C_DRIVER
	s32 (*init)(struct i2c_client *client);
	s32 (*sleep)(struct i2c_client *client);
	s32 (*wakeup)(struct i2c_client *client);
#endif
