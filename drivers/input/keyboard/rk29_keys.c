...
struct rk29_keys_drvdata {
	...
	bool in_weak_suspend;	/* Flag to indicate if we're idle */
	struct input_dev *input;
	struct adc_client *client;
	struct timer_list timer;
	struct rk29_button_data data[10];
...
void rk28_send_wakeup_batlow(void)
{
	if (!input_dev)
		return;
	input_report_key(input_dev, KEY_BAT_LOW, 1);
	input_sync(input_dev);
	input_report_key(input_dev, KEY_BAT_LOW, 0);
	input_sync(input_dev);
}
...
static irqreturn_t keys_isr(int irq, void *dev_id)
{
	...
	wake_lock_timeout(&adc_wake_lock, 3 * HZ);
...
{	
	struct rk29_keys_drvdata *ddata = container_of(work, struct rk29_keys_drvdata, work);
	adc_async_read(ddata->client);
}

static irqreturn_t adc_irqio_isr(int irq, void *dev_id)
{
	struct rk29_keys_drvdata *ddata=(struct rk29_keys_drvdata *)dev_id;
	wake_lock_timeout(&adc_wake_lock, 3 * HZ);
...
static int __devinit keys_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&ddata->work, adc_timer_work);
	...
	if(pdata->adc_irq_io){
		int adc_io_irq;
		if(pdata->adc_irq_io != INVALID_GPIO) {
			error = gpio_request(pdata->adc_irq_io, "adc_irq_io");
			if (error < 0) {
				pr_err("fail to request adc irq io\n");
				goto fail1;
			}
			error = gpio_direction_input(pdata->adc_irq_io);
			if (error < 0) {
				pr_err("adc_irq: failed to configure input"
					" direction for GPIO %d, error %d\n",
					pdata->adc_irq_io, error);
				gpio_free(pdata->adc_irq_io);
				goto fail1;
			}
			adc_io_irq = gpio_to_irq(pdata->adc_irq_io);
			if (adc_io_irq < 0) {
				error = adc_io_irq;
				pr_err("adc_irq_io: Unable to get irq number"
					" for GPIO %d, error %d\n",
					pdata->adc_irq_io, error);
				gpio_free(pdata->adc_irq_io);
				goto fail1;
			}
			error = request_irq(adc_io_irq, adc_irqio_isr,
					   IRQF_TRIGGER_RISING,
					   "adc_irq_io",
					    ddata);
			if (error) {
				pr_err("adc_irq_io: Unable to claim irq %d; error %d\n",
					adc_io_irq, error);
				gpio_free(pdata->adc_irq_io);
				goto fail1;
			}			
	...
	wake_lock_init(&adc_wake_lock, WAKE_LOCK_SUSPEND, "adkey_lock");	
	input_dev = input;
	return error;
...
static int keys_suspend(struct device *dev)
{
	...
	if (get_suspend_state() == PM_SUSPEND_IDLE) {
		/* to weak suspend */
		ddata->in_weak_suspend = true;
		for (i = 0; i < pdata->nbuttons; i++)
		{
			struct rk29_keys_button *button = &pdata->buttons[i];
			if (button->adc_value && pdata->adc_irq_io) {
				int irq = gpio_to_irq(pdata->adc_irq_io);
				enable_irq_wake(irq);
			} else {
