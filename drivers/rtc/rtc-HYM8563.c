...
{	
	...
	{	
		printk("[rtc] %s:diff_sec= %ds , use time\n",__func__, diff_sec);
		hym8563_enable_count(client, 1);				
		hym8563_set_count(client, diff_sec);
	}
	else
	{				
		printk("[rtc] %s:diff_sec= %ds , use alarm\n",__func__, diff_sec);
...
static int hym8563_rtc_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case RTC_AIE_OFF:
		printk("[rtc] close alarm.\n");
		if(hym8563_i2c_close_alarm(client) < 0)
			goto err;
		break;
	case RTC_AIE_ON:
		printk("[rtc] open alarm.\n");
