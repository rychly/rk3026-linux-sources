...
#if defined(CONFIG_MACH_RK3026_E601)
...
static int cyttsp4_power_on(int on)
{
	...
	if (!ldo_tp) {
		printk("cyttsp4 power not available.\n");
		return -1;
	}
	...
	if(on) {
		if (!regulator_is_enabled(ldo_tp)) {
			...
			printk("%s: cyttsp4 power enable\n", __func__);
		}
	} else {
		while(regulator_is_enabled(ldo_tp)>0) {
			regulator_disable(ldo_tp);
			printk("%s: cyttsp4 power disable\n", __func__);
		}
	}
...
void __init ebook_cyttsp4_init(void)
{
	...
	//ret = gpio_request(PIN_TOUCH_SWDL, "touch_not_used");
	...
	//gpio_direction_input(PIN_TOUCH_SWDL);
...
//free_rst:
	//gpio_free(CYTTSP4_I2C_RST_GPIO);
free_intb:
	gpio_free(CYTTSP4_I2C_IRQ_GPIO);
}

EXPORT_SYMBOL(ebook_cyttsp4_init);
