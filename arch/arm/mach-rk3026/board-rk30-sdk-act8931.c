...
static int act8931_set_init(struct act8931 *act8931)
{
	...
	for(i = 0; i < ARRAY_SIZE(act8931_dcdc_info); i++)
	{
		...
		if (act8931_dcdc_info[i].enable == 1)
		{
	...
	for(i = 0; i < ARRAY_SIZE(act8931_ldo_info); i++)
	{
		...
		if (act8931_ldo_info[i].enable == 1)
		{
...
int act8931_set_ldo(const char *id, bool enable)
{
	struct regulator *ldo=NULL;
	
	ldo = regulator_get(NULL, id);
	if (!ldo)
	{
		pr_err("%s: fail to set ldo %s\n", __func__, id);
		return 1;
	}
