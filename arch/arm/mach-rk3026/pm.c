...
static noinline void rk3026_pm_dump_irq(void)
{
...
static noinline void rk3026_pm_dump_inten(void)
{
	DUMP_GPIO_INTEN(0);
	DUMP_GPIO_INTEN(1);
	DUMP_GPIO_INTEN(2);
...
static u16 clkgt_first_w_msk[CRU_CLKGATES_CON_CNT] = {
	...
	(u16)~(0),
	(u16)~(0 | PM_HOLDGATE(PERIPH_SRC)
			| PM_HOLDGATE(PCLK_PERIPH)
			| PM_HOLDGATE(ACLK_PERIPH)),
	(u16)~(0),
	(u16)~(0 | PM_HOLDGATE(HCLK_PERI_AXI_MATRIX)
	...
	(u16)~(0 | PM_HOLDGATE(PCLK_GRF)
			| PM_HOLDGATE(PCLK_DDRUPCTL)),
	(u16)~(0),
	(u16)~(0 | PM_HOLDGATE(PCLK_PWM01)),
	(u16)~(0 | PM_HOLDGATE(PCLK_GPIO0)
			| PM_HOLDGATE(PCLK_GPIO1)
			| PM_HOLDGATE(PCLK_GPIO2)
			| PM_HOLDGATE(PCLK_GPIO3)),
	(u16)~(0 | PM_HOLDGATE(CLK_L2C)
...
static u16 __sramdata clkgt_sram_w_msk[CRU_CLKGATES_CON_CNT] = {
	...
	(u16)~(0 | PM_HOLDGATE(PERIPH_SRC)
			| PM_HOLDGATE(PCLK_PERIPH)),
	(u16)~(0),
	(u16)~(0 | PM_HOLDGATE(ACLK_STRC_SYS)
			| PM_HOLDGATE(ACLK_INTMEM)),
	(u16)~(0 | PM_HOLDGATE(PCLK_GRF)
			| PM_HOLDGATE(PCLK_DDRUPCTL)),
	(u16)~(0),
...
static u32 __sramdata sram_grf_uoc1_con0;
static void __sramfunc rk_pm_soc_sram_uart_suspend(void)
{
	sram_grf_uoc1_con0= grf_readl(GRF_UOC1_CON0);
       grf_writel(0x30000000, GRF_UOC1_CON0);
}
static void __sramfunc rk_pm_soc_sram_uart_resume(void)
{
       grf_writel(0x30000000 | sram_grf_uoc1_con0, GRF_UOC1_CON0);
}
...
static void __sramfunc rk3026_sram_suspend(int state)
{
	sram_printch('5');
	...
	if (PM_SUSPEND_MEM == state){
		...
		sram_printch('7');
		RK_SOC_PM_CTR_FUN(NO_UART, rk_pm_soc_sram_uart_suspend);
	...
	if (PM_SUSPEND_MEM == state){
		...
		RK_SOC_PM_CTR_FUN(NO_UART, rk_pm_soc_sram_uart_resume);
		sram_printch('6');
	}
	RK_SOC_PM_CTR_FUN(NO_DDR, ddr_resume);
	sram_printch('5');
}
static void noinline rk3026_suspend(int state)
{
	DDR_SAVE_SP(save_sp);
	rk3026_sram_suspend(state);
...
u32 __sramdata sram_pm_state;
static int rk3026_pm_enter(suspend_state_t state)
{
	//rk3026_pm_dump_inten();
...
static int rk3026_pm_prepare(void)
{
...
static void rk3026_pm_finish(void)
{
	enable_hlt();
}

static struct platform_suspend_ops rk3026_pm_ops = {
	.enter		= rk3026_pm_enter,
#if defined(CONFIG_IDLE)
	.valid		= suspend_valid_idle_mem,
#else
	.valid		= suspend_valid_only_mem,
#endif
	.prepare 		= rk3026_pm_prepare,
	.finish		= rk3026_pm_finish,
};

static int __init rk3026_pm_init(void)
{
	suspend_set_ops(&rk3026_pm_ops);
