...
static int dclk_lcdc_set_rate(struct clk *clk, unsigned long rate)
{
	if (rate == 27 * MHZ)
		return clkset_rate_freediv_autosel_parents(clk, rate);
			...
			int new_rate;
			int freq;
			if(clk->name == "dclk_lcdc1"){
				if(rate <= 75* MHZ)
					div = 5;
				else if(rate > 75* MHZ && rate <= 100* MHZ)
					div = 4;
				else if(rate > 100* MHZ && rate <= 125* MHZ)
					div = 3;
				else if(rate > 125* MHZ && rate <= 150* MHZ)
					div = 2;
				else 
					div = 1;
				for(freq = 800000000;freq > 300000000;){
				new_rate = freq / (div + 1);
				if (new_rate == rate){
...
static void __init rk30_clock_common_init(unsigned long gpll_rate, unsigned long cpll_rate)
{
	...
	if (!flag_uboot_display) {
		...
		#if defined(CONFIG_EBC)
			clk_set_parent_nolock(&dclk_lcdc1, &codec_pll_clk);
		#else
