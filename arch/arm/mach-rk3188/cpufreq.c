...
#endif
#ifdef CONFIG_ARCH_RK3026
	printk("rk3026 cpufreq version " VERSION ", suspend freq %d MHz\n", suspend_freq / 1000);
#else
...
static int rk_cpufreq_ebc_notifier_event(struct notifier_block *this, unsigned long event, void *ptr)
{
	int ret = NOTIFY_DONE;
	int i;
	struct cpufreq_policy *policy = cpufreq_cpu_get(0);
	...
	switch (event) {
	case EBC_ON:
		no_cpufreq_access = 0; //fix ebc two scarper to cpufreq_driver_target error
		ret = cpufreq_driver_target(policy, freq, DISABLE_FURTHER_CPUFREQ | CPUFREQ_RELATION_H);
		if (ret < 0) {
			ret = NOTIFY_BAD;
			goto out;
		}
		...
		break;
	case EBC_OFF:
		cpufreq_driver_target(policy, freq, ENABLE_FURTHER_CPUFREQ | CPUFREQ_RELATION_H);
		ret = NOTIFY_OK;
		break;
	}
out:
...
static struct notifier_block rk_cpufreq_ebc_notifier = {
	.notifier_call = rk_cpufreq_ebc_notifier_event,
};
static int __init rk3188_cpufreq_driver_init(void)
{
	register_pm_notifier(&rk3188_cpufreq_pm_notifier);
	#if defined(CONFIG_EBC)
	rkebc_register_notifier(&rk_cpufreq_ebc_notifier);
	#endif
