...
static int rk3026_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute) {
		if (rk3026_priv && rk3026_priv->hp_ctl_gpio != INVALID_GPIO &&
			...
			DBG("%s : set hp ctl gpio LOW\n", __func__);
			gpio_set_value(rk3026_priv->hp_ctl_gpio, GPIO_LOW);
			msleep(rk3026_priv->delay_time);	// 200
			}
		...
		if (rk3026_priv && rk3026_priv->hp_ctl_gpio != INVALID_GPIO &&
			...
			msleep(rk3026_priv->delay_time);	// 100
		}
	}
	return 0;
}
...
static int rk3026_codec_power_up(int type)
{
	...
	do {
		...
		retry++;	
		if (retry >= 30) {	// codec need reset
			printk("\n%s : ---------codec power_up failed, will be reset .\n", __func__);
			retry = 0;
			//codec power reset
			rk3026_plt->power_set(0);
			mdelay(50);
			rk3026_plt->power_set(1);
			//codec reg reset
			rk3026_reset(codec);
			#ifdef WITH_CAP
		...
		} else if ((retry >= 1) && (retry < 30)) {	// codec need powerup again
			mdelay(50);
		}

	}while(((RK3026_HPOUTL_MSK | RK3026_HPOUTR_MSK) & snd_soc_read(codec, RK3026_HPOUT_CTL)) == 0);
...
static int rk3026_codec_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	struct rk3026_codec_pdata  *rk3026_plt = codec->dev->platform_data;
	...
	// turn off codec power here 
	rk3026_plt->power_set(0);

	return 0;
}

static int rk3026_codec_resume(struct snd_soc_codec *codec)
{
	struct rk3026_codec_pdata  *rk3026_plt = codec->dev->platform_data;

	// turn on codec power here 
	rk3026_plt->power_set(1);
...
static struct snd_soc_codec_driver soc_codec_dev_rk3026 = {
	...
	.suspend =rk3026_codec_suspend,
	.resume = rk3026_codec_resume,
